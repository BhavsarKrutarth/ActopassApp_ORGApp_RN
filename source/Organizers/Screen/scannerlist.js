...
const Scannerlist = () => {
  ...
  const deletedata = async (id) => {
    ...
    try {
      const response = await deletescannerdata(id);
  ...
  const getscannerlist = async (value, Refresh) => {
    if (Refresh) {
      SetLoading(false);
    } else {
      SetLoading(true);
    ...
    try {
      const response = await getscanner(
        value || Pageindex,
        Pagecount,
        OrganizerLoginId
      );
